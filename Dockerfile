...
ARG GO_MODULE="github.com/stuttgart-things/sweatShop-creator"
ARG VERSION=""
ARG BUILD_DATE=""
ARG COMMIT=""
...
RUN go mod tidy
RUN CGO_ENABLED=0 go build -o /bin/sweatShop-creator \
...
COPY --from=builder /bin/sweatShop-creator /bin/sweatShop-creator

ENTRYPOINT ["sweatShop-creator"]
